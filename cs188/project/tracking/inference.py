...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particlelist = []
        for sample in range(self.numParticles):
            self.particlelist.append(self.legalPositions[sample % len(self.legalPositions)])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            if allPossible.totalCount() == 0:
                ...
                new = []
                for p in self.particlelist:
                    new.append(util.sample(allPossible))
                self.particlelist = new
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particlelist:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            new.append(util.sample(newPosDist))
        self.particlelist = new
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        particle = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(particle)
        self.particles = []
        for p in range(self.numParticles):
            self.particles.append(particle[p % len(particle)])
        ...
        for p in self.particles:
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] == None:
                    ...
                    prod *= emissionModels[ghost][trueDistance]
            allPossible[p] += prod
        allPossible.normalize()
        if allPossible.totalCount() == 0:
            self.initializeParticles()
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for i in range(len(newParticle)):
                ...
                new.append(util.sampleFromCounter(newPosDist))
            newParticle = new
            "*** END YOUR CODE HERE ***"
