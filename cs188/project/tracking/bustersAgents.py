...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        """Try to do it only using generators, just like above, LOL"""
        mostLikelyPos = [beliefs.argMax() for beliefs in livingGhostPositionDistributions]
        minDis = min([self.distancer.getDistance(pacmanPosition, pos) for pos in mostLikelyPos])
        closestGhost = [pos for pos in mostLikelyPos if self.distancer.getDistance(pacmanPosition, pos) == minDis][0]
        successorDis = [self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhost) for action in legal]
        return [action for action in legal if self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhost) == min(successorDis)][0]