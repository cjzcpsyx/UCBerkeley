        ...
        if Checksum.validate_checksum(ack):
            ...
            if not self.msg_type == 'end':
                for j in range(self.window_size - len(self.deliver_window)):
                    ...
                    if self.msg_type == 'end':
                        break
            for deliver_keys2 in self.deliver_window.keys():
                packet4 = self.make_packet(self.deliver_window[deliver_keys2][0], deliver_keys2, self.deliver_window[deliver_keys2][1])
                self.send(packet4)
